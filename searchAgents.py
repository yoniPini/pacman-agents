...
def manhattanHeuristic(position, problem, info={}):
    ...
    total = 0
    xy1 = position.getOnlyState()
    t = problem.goal
    if isinstance(t[0], int):
        return abs(xy1[0] - t[0]) + abs(xy1[1] - t[1])
    for xy2 in problem.goal:
...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        Meaning no more corners to visit or there is.
        """
        return (len(state[1]) == 0)
        util.raiseNotDefined()

    """
    check which corners left given the current state.
    """

    def which_corners_left(self, state) -> tuple:
        l = []
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                cost = self.costFn(nextState)
                corners_left = self.which_corners_left(nextState)
                successors.append(((nextState, corners_left), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    walls = problem.walls
    pos = state.getOnlyState()
    maxDistance = 0
    for c in corners:
        t = abs(pos[0] - c[0]) + abs(pos[1] - c[1])
        if t > maxDistance:
            maxDistance = t
...
def greatestDistance(l: list):
    point1 = l[0]
    point2 = l[1]
    maxD = abs(point1[0] - point2[0]) + abs(point1[1] - point2[1])
    for i in range(len(l)):
        for j in range(len(l)):
            d = abs(l[i][0] - l[j][0]) + abs(l[i][1] - l[j][1])
            if d > maxD:
                point1 = l[i]
                point2 = l[j]
                maxD = d
    return point1, point2, maxD
...
def foodHeuristic(state, problem):
    ...
    """
    position, foodGrid = state.getState()
    l = foodGrid.asList()
    if len(l) > 1:
        p1, p2, dis = greatestDistance(l)
        minD = min(abs(p1[0] - position[0]) + abs(p1[1] - position[1]),
                   abs(p2[0] - position[0]) + abs(p2[1] - position[1]))
        return dis + minD
    if len(l) == 1:
        return abs(l[0][0] - position[0]) + abs(l[0][1] - position[1])
    return 0
